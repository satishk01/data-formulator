...
def check_available_models():
    ...
    for provider in providers:
        ...
        if not models:
            # Provide default models for bedrock if none specified
            if provider == "bedrock":
                models = "anthropic.claude-3-5-sonnet-20241022-v2:0,anthropic.claude-3-5-haiku-20241022-v1:0,anthropic.claude-3-opus-20240229-v1:0,anthropic.claude-3-sonnet-20240229-v1:0,anthropic.claude-3-haiku-20240307-v1:0,amazon.nova-pro-v1:0,amazon.nova-lite-v1:0,amazon.nova-micro-v1:0"
            else:
                continue
...
def sanitize_model_error(error_message: str) -> str:
    ...
    # AWS Bedrock specific error mappings
    aws_error_mappings = {
        ...
        'AccessDeniedException': 'Access denied. Please check your AWS permissions for Bedrock',
        'ResourceNotFoundException': 'The specified model or resource was not found',
        'ServiceQuotaExceededException': 'Service quota exceeded. Please check your AWS limits',
        'NoCredentialsError': 'AWS credentials not found. Please provide valid credentials',
        'PartialCredentialsError': 'Incomplete AWS credentials. Please provide both access key and secret key',
        'ClientError': 'AWS service error. Please check your configuration and try again'
    }
    
    # Apply AWS error mappings
    for aws_error, friendly_message in aws_error_mappings.items():
        if aws_error in message:
    ...
    message = re.sub(r'(aws[-_]?access[-_]?key[-_]?id|aws[-_]?secret[-_]?access[-_]?key)[=:]\s*[^\s&]+', r'\1=<redacted>', message, flags=re.IGNORECASE)
